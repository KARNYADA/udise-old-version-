...
(function UDISEAutoFiller() {
  ...
  const processStudent = async (row, data) => {
    ...
    log(`  → Schooling Status select: "${preset.label}"...`);
    const schSet = setSelectByText(dd.schooling,
      preset.pattern, 'Schooling Status');
    log(`     ✓ "${schSet}"`);
    await sleep(CFG.delayAfterSelect);
    ...
    if (!preset.pattern.test(schNow))
      throw new Error(`Schooling verify fail: "${schNow}" (expected "${preset.label}")`);
    if (preset.needsSection) {
